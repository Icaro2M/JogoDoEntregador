...
# Lista de vértices conectados via neighbors corretamente
...
func _ready():
	...
	# 🔥 Sorteia ponto e desativa os outros
	randomize()
	var pontos = [
		get_node("../../Ponto"),
		get_node("../../Ponto2"),
	...
	var index = randi() % pontos.size()
	var ponto_escolhido = pontos[index]
	var goal = get_vertice_mais_proximo(ponto_escolhido.global_position)

	# Desativa os pontos e NPCs não sorteados
	for i in range(pontos.size()):
		var ativo = (i == index)
		pontos[i].monitoring = ativo
		pontos[i].monitorable = ativo
		pontos[i].visible = ativo
		npcs[i].visible = ativo

	print("🎯 Ponto escolhido como destino: ", ponto_escolhido.name)
...
func get_vertice_mais_proximo(pos: Vector3) -> Node3D:
	var menor_dist = INF
